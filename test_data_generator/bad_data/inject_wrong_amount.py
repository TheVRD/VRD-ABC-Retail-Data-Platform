# ==========================================================
# Module Name         : inject_wrong_amount
...
from test_data_generator.config.bad_data_config import WRONG_AMOUNT_VALUE_PERCENTAGE, AMOUNT_FIELD_NAME
from random import sample, randint
from dataclasses import replace


def inject_wrong_amount_value(
    ...
    if not 0 <= WRONG_AMOUNT_VALUE_PERCENTAGE <= 1:
        raise ValueError(f"{WRONG_AMOUNT_VALUE_PERCENTAGE} that is negative value percentage must be between 0 adn 1")
    ...
    wrong_amount_count = int(
        len(sales_records) * WRONG_AMOUNT_VALUE_PERCENTAGE
    )
    ...
    wrong_amount_count = min(
        wrong_amount_count,
        len(available_records)
    )

    records_to_insert_negative = set(sample(available_records, wrong_amount_count))

    for i in range(len(corrupted_records)):
        if corrupted_records[i] in records_to_insert_negative:
            field_to_insert_wrong_value = AMOUNT_FIELD_NAME
            wrong_val = randint(200, 1000000)
